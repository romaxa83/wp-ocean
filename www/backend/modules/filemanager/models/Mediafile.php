...
//        var_dump($absolutePath);die();
    ...
    {
        return self::find()->filterWhere(['in', 'id', $id])->all();
    }
    ...
    public static function create(ImageData $image_data) : self
    {
        $image = new static();
    ...
    {
        $file = self::find()->where(['id' => $id])->asArray()->one();
        $file['thumbs'] = unserialize($file['thumbs']);
        return $file;
    }

    public static function getThumbnailById($id, $thumbnail = 'original', $options = [])
    {
        $file = self::find()->where(['id' => $id])->asArray()->one();
        ...
        if($file) {
            if($thumbnail == 'original') {
                $src = Url::to('/admin/' . $file['url'], true);
            }
            else {
                $file['thumbs'] = unserialize($file['thumbs']);
                if(!array_key_exists($thumbnail, $file['thumbs'])) {
                    throw new Exception("Миниатюра по имени $thumbnail не существует");
                }
                $src = Url::to('/admin' . $file['thumbs'][$thumbnail]);
            }
            $alt = $file['alt'];

            $template = "<img src='$src' alt='$alt' {$class}>";
            return $template;
        }

        $src = Url::to('/admin/img/not-images.png', true);
        $template = "<img src='$src' alt='image not found' {$class}>";
