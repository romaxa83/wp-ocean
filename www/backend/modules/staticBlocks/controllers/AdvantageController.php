<?php

namespace backend\modules\staticBlocks\controllers;

use Yii;
use yii\base\Module;
use yii\helpers\Url;
use yii\web\Controller;
use yii\filters\VerbFilter;
use backend\modules\blog\helpers\StatusHelper;
use backend\modules\staticBlocks\forms\AdvantageForm;
use backend\modules\staticBlocks\services\StaticBlocksService;
use backend\modules\staticBlocks\forms\search\AdvantageSearch;
use backend\modules\staticBlocks\repository\StaticDataRepository;
use yii\filters\AccessControl;

class AdvantageController extends StaticBlocksController
{


    public function behaviors() {
        return [
            'access' => [
                'class' => AccessControl::className(),
                'rules' => [
                    [
                        'actions' => ['login', 'error'],
                        'allow' => true,
                    ],
                    [
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
            ],
            'verbs' => [
                'class' => VerbFilter::className(),
                'actions' => [
                    'delete' => ['POST'],
                ],
            ]
        ];
    }

    /**
     * @perm('Просмотр блока "Наши преимущества" (блоки на гл.)')
     */
    public function actionIndex()
    {
        $this->access->accessAction();
        $searchModel = new AdvantageSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'blocks' => $this->static_repository->getData('advantage'),
            'access' => $this->access
        ]);
    }

    /**
     * @perm('Редактирование блока "Наши преимущества" (блоки на гл.)')
     */
    public function actionUpdate($id)
    {
        $this->access->accessAction();
        $block = $this->static_repository->get($id);
        $form = new AdvantageForm($block);
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            try {
                $this->static_service->edit($id, $form);
                return $this->redirect(['index']);
            } catch (\DomainException $e) {
                $this->error($e);
            }
        }
        return $this->render('update', [
            'model' => $form,
        ]);
    }
}
